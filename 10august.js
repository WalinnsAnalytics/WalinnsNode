													...
													/* console.log(rows1[i].push_server_key); */
													   ...
													   connection.query('SELECT push_token from deviceinfodatas, uninstallcounts where deviceinfodatas.id = uninstallcounts.deviceid and push_token not in ("BLACKLISTED","null") and push_notification_status = ? and project_token = ?', ['true', rows1[i].project_token ],function(err, rows, fields){
															...
															if(err) {
																...
																/* console.log("Push Token Project specific :",push_token_set); */
																
																 for(var l = 0;  l < obj.results.length;l++){
																	(function(h){
																		/* console.log(obj.results[l]); */
																...
																console.log("rtgertretre");
																console.log(uninstall_tokens);
																var values = [];
																connection.query('SELECT deviceid from deviceinfodatas, uninstallcounts where deviceinfodatas.id = uninstallcounts.deviceid and push_token in (?)', [c],function(err, row2, fields){
																			
																			
																			
																			for(var i = 0; i < row2.length;i++){
																				console.log(row2[i].deviceid);
																				r = row2[i].deviceid;
																				connection.query('SELECT deviceid, push_token  from deviceinfodatas, uninstall_history where deviceinfodatas.id = uninstall_history.deviceid and deviceid = ? ', [r],function(err, row3, fields){
																				
																				if(row3.length != 0)
																				{
																					if(row3[0].push_token !==  c[i]) {
																						values.push([row3[0].deviceid, moment().format("YYYY-MM-DD HH:mm:ss"), c[i]]);
																					}	
																				}
																				if(row3.length == 0)
																				{	
																					/* console.log("ggggggggggggggggggggggggggggggggggggggggggg"); */
																					values.push([r, moment().format("YYYY-MM-DD HH:mm:ss"), c[i]]);
																					console.log(values);
																				}
